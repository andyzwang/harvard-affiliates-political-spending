...
ggplot(gender_race_school_data, aes(x = race, y = donations_sum, color = gender)) +
  geom_jitter(width = .25, height = .1) +
  scale_y_continuous(labels = scales::dollar)

ggplot(gender_race_school_data, aes(x = school, y = donations_sum, color = gender)) +
  geom_jitter(width = .25, height = .1) +
  scale_y_continuous(labels = scales::dollar)

gender_race_school_model <- lm(donations_sum ~ gender*race + school, gender_race_school_data) %>%
  tidy(conf.int = T)
```
