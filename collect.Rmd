...
# joining tables of individual donations with committee information, to be able to sort by party. group together by party, name, title, etc, and then summarize.
...
ggplot(spending_over_time, aes(x = month, y = spending, color = as.factor(cycle))) +
  geom_line() +
  labs(
    x = "Date",
    y = "Total Spending",
...
```{r unimodel}
# creating a linear model for gender, race, and school, with one interaction term
...
ggplot(gender_race_subject_data, aes(x = field, y = spending_sum, color = gender)) +
...
ggplot(gender_race_subject_data, aes(x = department, y = spending_sum, color = gender)) +
  geom_jitter(width = .1, height = .25) +
  coord_flip() +
  scale_y_log10(labels = scales::dollar)
